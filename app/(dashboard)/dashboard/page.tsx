...
export default async function DashboardPage() {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    chartData.push({
      ...
      responses: allResponses.filter((r) => r.submittedAt >= day && r.submittedAt < nextDay).length,
    });
  }
  ...
  const formListData = forms.map((f) => ({
    id: f.id,
    title: f.title,
